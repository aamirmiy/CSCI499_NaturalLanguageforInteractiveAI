...
def setup_dataloader(args):
    ...
    return vocab_to_index, train_loader, val_loader, (actions_to_index, index_to_actions, targets_to_index, index_to_targets)
...
def add_new_word(new_vector,new_index,embedding_matrix):
    """
...
def setup_model(device, output_size1, output_size2, embedding_dim,embedding_matrix, hidden_dim, n_layers):
    """
    return:
    ...
    # ===================================================== #
    model = semanticNet(device, output_size1, output_size2, embedding_dim,embedding_matrix, hidden_dim, n_layers)
...
def main(args):
    ...
    # get dataloaders
    v2i,train_loader, val_loader, maps = setup_dataloader(args)
    ...
    embedding_matrix = create_embedding_matrix(v2i, glove_embedding, 300)
    #Adding UNK,PAD,SOS,EOS tokens in embedding matrix
    pad_index = 0
    start_index = 1
    end_index = 2
    unk_index = 3
    eos_vector = np.random.standard_normal(300)
    sos_vector = np.random.standard_normal(300)
    unk_vector = embedding_matrix.mean(0)
    pad_vector = np.zeros(300)
     
    embedding_matrix = add_new_word(eos_vector,end_index,embedding_matrix)
    embedding_matrix = add_new_word(sos_vector,start_index,embedding_matrix)
    embedding_matrix = add_new_word(unk_vector,unk_index,embedding_matrix)
    embedding_matrix = add_new_word(pad_vector, pad_index, embedding_matrix)
    # build model
    model = setup_model(device,len(maps[0]),len(maps[2]),300,embedding_matrix,256,1)
